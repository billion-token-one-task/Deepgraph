...
硬件事实 (2026-08-06 实测): **8 张 NVIDIA A100-PCIE-40GB**, 分在同一台主机的两个 SSH 端口,
`111.172.214.101:32035` 与 `:32036`, 各 4 张; 宿主 CPU 是 aarch64 (Kunpeng-920), 驱动 570.86.10,
...
告诉对方这几件事就够了 (**不要直接给密码, 给读取位置**):

- 目标: `ssh -p 32035 root@111.172.214.101` (另一台是 `-p 32036`), 每台 4 张 A100-40GB;
- 凭据: 部署机 `/home/billion-token/Deepgraph/.env` 里的 `DEEPGRAPH_SSH_CREDENTIAL`
  (需要 sudo 读; 或由运维直接注入到对方的 SSH_ASKPASS/agent, 不落盘);
...
> 你有 8 张 A100-40GB, 在 `111.172.214.101` 的 22 端口以外的两个端口 `32035` 和 `32036`
> (各 4 张), 用户 root, 密码请向运维索取或从部署机 .env 的 `DEEPGRAPH_SSH_CREDENTIAL` 读取。
...
- **两个端口是同一台物理机**: 8 张卡共享同一套 CPU/内存/磁盘, 大规模数据预处理会互相拖慢。
