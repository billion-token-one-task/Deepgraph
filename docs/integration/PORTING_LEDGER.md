...
| 3 | both | legacy high-cost `call_llm` callers | proposal/forge/validation/manuscript role adapters | stable proposal identity plus scoped ResourceGrant; proposer/evaluator/reviewer provenance and independence | core paths adapted; residual inventory and CI remain | in_progress |
...
| 4 | `6048a95` | SSH/GPU schedulers | durable scheduler admission plus guarded legacy transport | ResourceGrant on GPU queues; terminal failures require measured usage; startup recovery is per agenda | legacy worker branches and runtime CI remain | in_progress |
