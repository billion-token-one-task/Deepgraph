...
| 6 | new contract | Frontier/Decision/Grant/Outcome | evidence-graph source, contracts, trusted assembler, repository, migration | evidence arrays come from scoped graph; auditable estimates, grants and actual metered outcomes | static/policy, PostgreSQL compute/evidence rerun, and trusted CPU/A100 OutcomeRecord canary passed; API/provider/reviewer remains open | adapted_needs_ci |
