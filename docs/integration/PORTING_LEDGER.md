...
| 2 | `7d0b42a` + `6048a95` | Agenda schema and missing problem/benchmark tables | `0001_meta_harness_v1.sql` | add-only, checksum journal, rerunnable, old backlog remains unscoped | fresh local backup restore and original local `deepgraph` cutover both passed first/second migration; all 58 original counts, FK/orphan/scope audit and 6/6 PostgreSQL repository integration are clean | adapted |
| 2 | `aed152a` | guarded original-local cutover | `scripts/meta_harness_migration.py`, `MIGRATION_RUNBOOK.md` | live writes require explicit env opt-in, exact `127.0.0.1:5433/deepgraph`, inactive service, verified `/home/ec2-user` backup and distinct confirmation | unit guard tests; custom backup/list/SHA; original local journal, service health and retained rollback config recorded 2026-08-02 | adapted |
...
| 6 | new contract | Frontier/Decision/Grant/Outcome | evidence-graph source, contracts, trusted assembler, repository, migration | evidence arrays come from scoped graph; auditable estimates, grants and actual metered outcomes | static/policy, PostgreSQL compute/evidence, API, real provider, reviewer and trusted CPU/A100 OutcomeRecord canaries passed | adapted |
