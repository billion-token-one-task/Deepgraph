...
PROBLEM_SHARPENING_SYSTEM = """You are a senior ML researcher identifying SHARP, FORMAL research problems from evidence of contradictions, performance plateaus, and recurring limitations across thousands of papers.
...
Return 6-12 problems. Quality over quantity. A problem without specific numbers is NOT a problem."""
...
def _build_problem_prompt(signals: dict) -> str:
    ...
    if signals["high_potential_insights"]:
        ...
        for ins in signals["high_potential_insights"][:5]:
            ...
            sections.append(f"  Hypothesis: {ins['hypothesis'][:200]}")
            sections.append(f"  Paradigm score: {ins.get('paradigm_score', 0)}")
            sections.append("")
...
def discover_paper_ideas(
    ...
    *,
    ...
    has_signals = (signals["contradiction_clusters"] or signals["performance_plateaus"]
                   or signals["limitation_clusters"] or signals["high_potential_insights"])
    if not has_signals:
    ...
    print(f"[PAPER_IDEA] {len(problems)} problems extracted", flush=True)
