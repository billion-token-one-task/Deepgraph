...
def _jsonify_provenance(value) -> str | None:
    if value is None:
        return None
...
def store_insight(insight: dict) -> int:
    ...
    src_nodes = insight.get("source_node_ids")
    if src_nodes is None and insight.get("node_id"):
        src_nodes = json.dumps([insight["node_id"]])
    else:
        src_nodes = _jsonify_provenance(src_nodes)
    ...
    rid = db.insert_returning_id(
        """INSERT INTO insights
           (node_id, insight_type, title, hypothesis, evidence, experiment, impact,
            ...
            generation_run_id, source_signal_ids, source_paper_ids, source_node_ids,
            prompt_version, model_version, exemplars_used, token_cost_usd, wall_clock_seconds,
            outcome)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
           RETURNING id""",
        (
            ...
            supporting,
            insight.get("generation_run_id") or new_generation_run_id(),
            _jsonify_provenance(insight.get("source_signal_ids")),
            _jsonify_provenance(insight.get("source_paper_ids")),
            src_nodes,
            insight.get("prompt_version") or PROMPT_VERSION,
            insight.get("model_version") or LLM_MODEL,
            _jsonify_provenance(insight.get("exemplars_used")),
