...
def get_node_papers(node_id: str, limit: int = 50) -> list[dict]:
    ...
    rows = db.fetchall(
        """SELECT p.id, p.title, p.authors, p.published_date, p.status,
                  ...
                  MAX(pi.plain_summary) as plain_summary,
                  MAX(pi.problem_statement) as problem_statement,
                  MAX(pi.approach_summary) as approach_summary,
                  MAX(pi.work_type) as work_type,
                  MAX(pi.key_findings) as key_findings,
                  MAX(pi.limitations) as limitations,
                  MAX(pi.open_questions) as open_questions
           FROM papers p
           JOIN paper_taxonomy pt ON p.id = pt.paper_id
           JOIN taxonomy_nodes t ON pt.node_id = t.id
           LEFT JOIN paper_insights pi ON p.id = pi.paper_id
           WHERE t.id = ? OR t.id LIKE ? || '.%'
           GROUP BY p.id, p.title, p.authors, p.published_date, p.status
...
def assign_paper_to_node(paper_id: str, node_id: str, confidence: float = 1.0):
    ...
    db.execute(
        """INSERT INTO paper_taxonomy (paper_id, node_id, confidence)
           VALUES (?, ?, ?)
           ON CONFLICT (paper_id, node_id) DO UPDATE SET
...
def insert_result(result: dict) -> int:
    ...
    metric_value = _coerce_optional_float(result.get("metric_value"))
    grounding_score = _coerce_optional_float(result.get("grounding_score"))
    payload = {
        ...
        "grounding_source_field": result.get("grounding_source_field"),
    }
    result_key = hashlib.sha256(json.dumps(payload, sort_keys=True, ensure_ascii=False).encode("utf-8")).hexdigest()
    existing = db.fetchone("SELECT id FROM results WHERE result_key=?", (result_key,))
    params = (
        result["paper_id"],
        result.get("node_id"),
        result["method_name"],
    ...
    if existing:
        ...
        db.execute(
            """UPDATE results
               SET paper_id=?, node_id=?, method_name=?, dataset_name=?, metric_name=?,
        ...
        rid = db.insert_returning_id(
            """INSERT INTO results
               (paper_id, node_id, method_name, dataset_name, metric_name,
                metric_value, metric_unit, is_sota, evidence_location, conditions,
                source_quote, char_start, char_end, grounding_status, grounding_score, grounding_source_field, result_key)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
               RETURNING id""",
            params,
        )
...
def assign_result_to_node(result_id: int, node_id: str, commit: bool = True):
    ...
    db.execute(
        """INSERT INTO result_taxonomy (result_id, node_id)
           VALUES (?, ?)
...
def get_recently_created_nodes(limit: int = 20) -> list[dict]:
    ...
    return db.fetchall(
        """SELECT t.*,
           ...
           FROM taxonomy_nodes t
           LEFT JOIN (
               SELECT node_id, COUNT(DISTINCT paper_id) AS paper_count
               FROM paper_taxonomy
               GROUP BY node_id
           ) pc ON pc.node_id = t.id
