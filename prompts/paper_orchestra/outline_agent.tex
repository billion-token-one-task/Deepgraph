...
\begin{itemize}
    ...
    \item Benchmark-first rule: if \texttt{experimental\_log.md} contains per-method metrics, seed results, standard deviations, budget histograms, ablations, or reproduction summaries, the first figures MUST be real evidence plots drawn from those data. Prefer method-comparison panels, error bars, ablation curves, seed variability plots, budget-allocation plots, and keep/discard search dynamics.
    \item Use at most one conceptual framework diagram in the main paper. Do not fill the plotting plan with decorative pipeline diagrams when benchmark evidence is available.
    \item Every plot objective must name the exact evidence it will display, such as metric names, compared methods, seeds, budgets, or ablation variables. Avoid vague objectives like "illustrate the framework" unless it is the single conceptual diagram.
...
\begin{itemize}
    \item Hypotheses: Define the "Hook" (broad context) and "Problem Gap" to be verified. CRITICAL: Strictly scope the problem gap and claims to match the specific datasets, modality (text/video/audio), and evaluations present in \texttt{experimental\_log.md}. Do not over-claim generalization. If the log only has text QA benchmarks, the outline must NOT frame the paper as video temporal grounding or moment retrieval.
