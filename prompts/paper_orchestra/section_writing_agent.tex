...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Follow \texttt{prompts/experiment\_table\_requirements.md}: booktabs three-line tables only (no vertical rules); compact abbreviated headers; $\checkmark$/\textendash{} for booleans; derived columns (CI, $\Delta$, Rel.); qualitative text in body, not in cells; subtle \texttt{\textbackslash{}rowcolor} for key rows---avoid blanket \texttt{\textbackslash{}textbf} on every best value; \texttt{tabularx} with \texttt{@\{\textbackslash{}extracolsep\{\textbackslash{}fill\}\}} for full-width tables; demote config/hyperparam lists to Setup prose.
