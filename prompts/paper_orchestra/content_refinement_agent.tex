...
\begin{enumerate}
    ...
    \item \textbf{Conference polish:} The final paper must use an abstract environment rather than a numbered Abstract section, must include a bibliography style before \textbackslash{}bibliography, and must only reference figure files that are present in the supplied figure list.
    \item \textbf{Evidence tone:} Avoid repeatedly saying "available log", "supplied artifact", or "provided materials" in the main body. Keep limitations concise and centralized, while preserving calibrated claims.
    \item \textbf{Human-paper standard:} Make the draft read like a compact conference paper: contribution list in the Introduction, clear problem gap, method equations only when grounded, dense results paragraphs, and no internal workflow narration.
    \item \textbf{Figures:} Prefer real benchmark panels, seed/error-bar plots, ablation curves, and budget allocation plots. Keep at most one conceptual diagram in the main narrative and ensure every figure is discussed with a concrete takeaway.
        \item \textbf{Citations:} Use ONLY keys from \texttt{citation\_map.json}. Do not use loosely related citations as analogies; each citation must directly support a task, method family, benchmark, or decision-theoretic framing.
    \end{itemize}

    \item \textbf{Section-Specific Revision Standard}
    \begin{itemize}
        \item \textbf{Abstract:} Enforce the order background trend, concrete problem, existing-method limitation, proposed method, core mechanism, and main results. First two sentences must be direct. Give full method name and abbreviation on first mention. Remove unsupported broad claims, non-core weakening details, and excessive punctuation. If evidence is controlled, subset-based, materialized, or a case study, keep that boundary clear.
        \item \textbf{Introduction:} Ensure it explains importance, existing-method insufficiency, paper objective, method response, short results, and three or four contributions. If Problem I/II/III framing is used, every problem must be a real failure mode and must be explicitly answered by a matching Method design.
        \item \textbf{Contributions:} Keep three or four bullets, each starting with a strong verb and claiming only completed work. Remove "first", "comprehensive", and "extensive" framing unless directly supported.
        \item \textbf{Related Work:} Make each subsection a short Title Case noun phrase, organize citations by category rather than as citation dumps, and end each subsection with the gap relative to this paper.
        \item \textbf{Method:} Keep three to five coherent subsections. Ensure every important formula explains inputs, outputs, variables, term purposes, and the problem it addresses. Remove result numbers, baseline criticism, significance analysis, TODOs, and undefined variables from Method.
    \end{itemize}
\end{enumerate}
