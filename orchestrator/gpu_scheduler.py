...
def _worker_ids(workers: list[dict] | None = None) -> list[str]:
    source = workers if workers is not None else list_workers()
    return [str(w.get("id") or "") for w in source if w.get("id")]


def recover_stale_running_jobs(workers: list[dict] | None = None) -> int:
    """Requeue GPU jobs left running by a controller restart.

    Background worker threads are in-process, so after a fresh scheduler start
    any ``gpu_jobs.status='running'`` row owned by this host has no live
    Python thread behind it.  Requeue it so ``run_validation_loop`` can resume
    from saved iteration state.
    """
    ids = _worker_ids(workers)
    if not ids:
        return 0
    ...
    for job in stale_jobs:
        ...
        if _current_run_is_successful(run_id):
            ...
            db.execute(
                ...
                """,
                (
                    "Recovered stale running job after scheduler restart; "
...
_SENSITIVE_METADATA_KEYS = {"password", "token", "api_key", "secret"}
...
def _claim_idle_worker(job: dict | None = None) -> dict | None:
    ...
    vram_sql = ""
    if required_vram > 0:
        vram_sql = " AND COALESCE(total_mem_gb, 0) >= ?"
        params.append(required_vram)
    workers = db.fetchall(
        f"""
        SELECT * FROM gpu_workers
        WHERE status='idle'
          AND hostname=?{vram_sql}
          AND NOT EXISTS (
              SELECT 1 FROM gpu_jobs gj
              WHERE gj.assigned_worker=gpu_workers.id
                AND gj.status='running'
...
def _maybe_recover_stale_jobs() -> int:
    ...
    return recover_busy_workers_without_running_jobs()
...
def start() -> dict:
    ...
    with _scheduler_lock:
        ...
        recovered = (
            recover_stale_running_jobs(workers)
            + recover_busy_workers_without_running_jobs()
        )
        _stop_event.clear()
